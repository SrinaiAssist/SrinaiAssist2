...
async function sendPushToUsers(usernames, { title, body, data, sound, channel }, excludeUsername) {
  ...
  await Promise.all(
    tokenRows.map(async (row) => {
      try {
        const result = await sendFcmMessage(row.token, { title, body, data, sound, channel });
