...
// Ambil fileId Drive dari URL yang disimpan di DB (format
// "https://drive.google.com/uc?export=download&id=<fileId>" untuk PDF, atau
// "https://drive.google.com/thumbnail?id=<fileId>&sz=..." untuk foto).
// Return null kalau bukan URL Drive (mis. fallback base64 lama).
...
// Hapus 1 file dari Google Drive berdasarkan fileId. Dipakai saat BA dihapus
// dari halaman SOS (sos.html) supaya file yang sudah tidak dipakai tidak
// numpuk terus menggerus kuota Drive. Best-effort: kalau gagal (file sudah
// terhapus manual sebelumnya, permission berubah, dll.) cukup dilempar
// errornya ke pemanggil supaya bisa di-log, TIDAK boleh menggagalkan proses
...
async function deleteFileFromDrive(fileId) {
  ...
  // 404 dianggap sukses juga (file memang sudah tidak ada di Drive).
  if (!resp.ok && resp.status !== 404) {
    const text = await resp.text().catch(() => '');
    throw new Error(`Hapus file Drive gagal (status ${resp.status}): ${text || 'unknown error'}`);
  }
}
